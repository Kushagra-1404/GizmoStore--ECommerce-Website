...
//User and Product are models of collections defined on mongodb atlas
...
router.post('/', async(req, res)=> {
  try {
    ...
    //sending back all the products after new
...
router.patch('/:id', async(req, res)=> {
  ...
  try {
    ...
    const products = await Product.find();
    //sending all the products back after update
    res.status(200).json(products);
  } catch (e) {
    res.status(400).send(e.message);
  }
...
router.delete('/:id', async(req, res)=> {
  ...
  try {
    ...
    const products = await Product.find();
    //sending updated products back to update in frontend
    res.status(200).json(products);
  } catch (e) {
    res.status(400).send(e.message);
  }
...
//to get one product along with similar products
router.get('/:id', async(req, res)=> {
  const {id} = req.params;
  try {
    const product = await Product.findById(id);
    //to show similar products of same category
...
router.post('/add-to-cart', async(req, res)=> {
  ...
  try {
    ...
    //increasing count of that product
    ...
    await user.save();
    //sending back updated user with cart
